...
def _mixed_h() -> FermiHamiltonian:
    return FermiHamiltonian({((1, 1), (0, 0)): -1.0 + 0j, ((0, 1), (0, 0)): 2.0 + 0j},
                            n_qubits=8, devices=["localhost:cuda:0"])

def _two_site_fermion_h() -> FermiHamiltonian:
    return FermiHamiltonian({((1, 1), (0, 0)): -1.0 + 0j, ((0, 1), (1, 0)): -1.0 + 0j},
...
def test_cuda_diagonal_only_hamiltonian() -> None:
    h = FermiHamiltonian({((0, 1), (0, 0)): 1.0 + 0j, ((1, 1), (1, 0)): 2.0 + 0j},
                         n_qubits=8, devices=["localhost:cuda:0"])
...
def test_cuda_apply_forward() -> None:
    ...
    m = prepare({((1, 1), (0, 0)): -1.0 + 0j}, n_qubits=4)
    assert jnp.allclose(h.apply_within_subspace(c, pi, c, direction=0),
                        jax_apply_within(c, pi, c, *m, direction=0), rtol=1e-12)

def test_cuda_apply_backward() -> None:
    h, c = _hopping_h(), _c4()
    pi = jnp.ones((4, 2), dtype=jnp.float64)
    m = prepare({((1, 1), (0, 0)): -1.0 + 0j}, n_qubits=4)
    assert jnp.allclose(h.apply_within_subspace(c, pi, c, direction=1),
                        jax_apply_within(c, pi, c, *m, direction=1), rtol=1e-12)
...
def test_cuda_apply_numerical_hopping() -> None:
    ...
    assert jnp.allclose(h.apply_within_subspace(ci, pi, cj, direction=0),
                        jax_apply_within(ci, pi, cj, *m, direction=0), rtol=1e-12)
...
def test_cuda_apply_identity_configs() -> None:
    ...
    pi = jnp.array([[1.0, 0.0]], dtype=jnp.float64)
    m = prepare({((1, 1), (0, 0)): -1.0 + 0j, ((0, 1), (1, 0)): -1.0 + 0j}, n_qubits=4)
    assert jnp.allclose(h.apply_within_subspace(ci, pi, ci, direction=0),
...
def test_cuda_apply_complex_psi() -> None:
    ...
    m = prepare({((1, 1), (0, 0)): 1.0 + 0j}, n_qubits=4)
    assert jnp.allclose(h.apply_within_subspace(ci, pi, cj, direction=0),
                        jax_apply_within(ci, pi, cj, *m, direction=0), rtol=1e-12)  # FIXME: CUDA sign bug)
...
def test_cuda_find_topk_max_semantics() -> None:
    h = FermiHamiltonian({((1, 1), (0, 0)): -1.0 + 0j, ((2, 1), (1, 0)): -0.5 + 0j},
                         n_qubits=4, devices=["localhost:cuda:0"])

...
`create_network` 的分发/反序列化/元数据注入由 `tests/unit/models/test_create_network.py` 直接覆盖（name 分发、未知 name `KeyError`、dacite 默认/覆盖/错类型/未知键、`electron_number` 驱动守恒、PRNG 确定性）；端到端调用由 `tests/unit/test_cli.py` 覆盖。

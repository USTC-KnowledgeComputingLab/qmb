    ...
    for (std::int64_t t = 0; t < term_number; ++t)
        for (std::int64_t b = 0; b < src_batch_size; ++b)
            apply_within_subspace_in_double_side_kernel<n_qubytes, particle_cut, max_op_number, forward>(
                t, b, dst_batch_size, site, kind, coef, src_configs, src_psi, sorted_dst_configs, result_psi);
}

void separate_interleaved_to_block(
    const std::uint8_t* interleaved, std::int64_t batch_size, std::int64_t n_qubytes,
    std::uint8_t* block, std::int64_t n_up)
{
    std::int64_t n_total_orbitals = n_qubytes * 8;
    for (std::int64_t b = 0; b < batch_size; ++b) {
        const std::uint8_t* src = interleaved + b * n_qubytes;
        std::uint8_t* dst = block + b * n_qubytes;
        memset(dst, 0, n_qubytes);
        for (std::int64_t i = 0; i < n_total_orbitals; ++i) {
    ...
    int64_t direction) -> torch::Tensor
{
    ...
    // Separate interleaved configs to block order
    ...
    separate_interleaved_to_block(configs_i.data_ptr<std::uint8_t>(), batch_i, n_qubytes,
                                   block_i.data_ptr<std::uint8_t>(), n_up);
    separate_interleaved_to_block(configs_j.data_ptr<std::uint8_t>(), batch_j, n_qubytes,
                                   block_j.data_ptr<std::uint8_t>(), n_up);
    ...
    if (direction == 0) {
        ...
        if (configs_j_sorted) {
            ...
            std::tie(sorted_j, sort_j_idx) = sort_configs<n_qubytes, particle_cut, max_op_number>(block_j);
        }
        ...
        if (configs_i_sorted) {
            ...
            std::tie(sorted_i, sort_i_idx) = sort_configs<n_qubytes, particle_cut, max_op_number>(block_i);
        }
...
TORCH_LIBRARY_IMPL(QMP_LIBRARY(N_QUBYTES, PARTICLE_CUT, MAX_OP_NUMBER, N_UP), CPU, m) {

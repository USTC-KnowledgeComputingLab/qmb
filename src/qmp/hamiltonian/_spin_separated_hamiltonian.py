...
class SpinSeparatedHamiltonian:
    _compiled_modules: ClassVar[dict[tuple[str, int, int, int, int], object]] = {}
    ...
    def __init__(
        ...
        *,
        ...
        self._n_down = self._compute_n_total(hamiltonian) - n_up
        self._site, self._kind, self._coef = self._prepare(hamiltonian, n_up, self._max_op_number)
    ...
    @staticmethod
    def _compute_n_total(hamiltonian: dict) -> int:
        max_site = 0
    ...
    def _load_module(
        ...
        if not is_declaration:
            cls._load_module("declaration", 0, 0, 0, 0)
        key = (device_type, n_qubytes, particle_cut, max_op_number, n_up)
        name = (
            "qmp_spin_separated_hamiltonian"
            if is_declaration
            else f"qmp_spin_separated_hamiltonian_{n_qubytes}_{particle_cut}_{max_op_number}_{n_up}"
        )
        if key not in cls._compiled_modules:
            ...
            match device_type:
                case "declaration":
                    sources = [os.path.join(folder, "_spin_separated_hamiltonian.cpp")]
                case "cpu":
                    sources = [
                        os.path.join(folder, "_spin_separated_hamiltonian.cpp"),
                        os.path.join(folder, "_spin_separated_hamiltonian_cpu.cpp"),
                    ]
                case "cuda":
                    sources = [os.path.join(folder, "_spin_separated_hamiltonian_cuda.cu")]
                case _:
    ...
    @classmethod
    def _prepare(cls, hamiltonian: dict, n_up: int, max_op_number: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        return cls._load_module().prepare(hamiltonian, n_up, max_op_number)
    ...
    def apply_within_subspace_in_double_side(
        ...
        *,
        ...
        op_module = self._load_module(
            self._device.type, configs_i.size(1), self._particle_cut, self._max_op_number, self._n_up
        )
        op = op_module.spin_separated_apply_within_subspace_in_double_side

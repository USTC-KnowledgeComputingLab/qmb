...
- `generate` / `generate_unique` 显式接收 PRNG `key`（纯函数，无内部随机状态）。
- 生成循环用 unrolled Python for（sites 静态已知，jit trace 期展开）；共享逻辑（mask、归一化、Gumbel top-K 束搜索）在 `networks/_autoregressive.py`。

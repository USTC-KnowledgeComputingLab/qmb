...
def _parity(pc: Array, pm: Array, config: Array, Q: int) -> int:
    p = int(pc) & 1
    for q in range(Q):
        p ^= jnp.bitwise_count(jnp.uint32(int(pm[q]) & int(config[q]))) & 1
    return p
...
def _try_register_ffi() -> bool:
    """Attempt to load and register CUDA FFI targets. Returns True if successful."""
    try:
        from ._hamiltonian_cuda_loader import load_cuda_module
        lib = load_cuda_module(n_qubytes=0, max_op_number=4)
        targets = {
            "qmp_compute_diagonal_within_subspace": "ComputeDiagonalWithinSubspace",
            "qmp_apply_within_subspace": "ApplyWithinSubspace",
            "qmp_find_all_relative_configs": "FindAllRelativeConfigs",
            "qmp_find_topk_relative_configs": "FindTopKRelativeConfigs",
        }
        for name, sym in targets.items():
            handler = getattr(lib, sym)
            jax.ffi.register_ffi_target(name, jax.ffi.pycapsule(handler), platform="CUDA")
        logger.info("CUDA FFI targets registered successfully.")
        return True
    except Exception:
...
_ffi_available: bool = _try_register_ffi()
...
class FermiHamiltonian:
    ...
    def _to_dev(self, arr: jax.Array | np.ndarray) -> jax.Array:
        if isinstance(arr, np.ndarray):
            return jax.device_put(jnp.asarray(arr), self._device)

...
各 Task 的内联测试块是最小 TDD 版本 (Task 1/2/4/5 分别 2/7/3/5 个用例)。代码稳定后测试被系统性扩充，覆盖边界、错误路径、不变量、端到端数值与网络注册 (Task 7)。以实际提交的测试文件为准，本计划的内联块不再逐字同步。当前覆盖 (models 部分 99 个用例)：
...
- **test_create_network.py**：`create_network` 按 name 分发四个注册配置（构造归一化）、未知 name `KeyError`、dacite 反序列化（空参用默认 / 覆盖 / 错类型 DaciteError / 未知键忽略）、model 元数据注入（`electron_number` 驱动守恒）、PRNG 确定性。

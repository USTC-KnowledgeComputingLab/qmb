...
**Architecture:** 每个 model 是薄封装：`ModelConfig` dataclass → `__init__` 构造哈密顿量字典 → `FermiHamiltonian(dict, n_qubits=..., devices=...)` → 转发四个算符方法 + `show_config` + 元数据。全局 `model_dict` + `model_config_dict` 注册表实现插件式发现。每个 model 文件底部另定义网络配置 dataclass 并注册进 `Model.network_dict`（见 Task 7）。
...
├── __init__.py
├── test_model.py
├── test_build.py          # SubConfigRef + build_model
├── test_create_network.py # model.create_network 分发 (Task 7)
├── test_hubbard.py
├── test_fcidump.py
└── test_openfermion.py
```
...
and ``model_config_dict`` are the plugin registries; models register
themselves at import time via ``model_dict[name] = Model`` and
``model_config_dict[name] = ModelConfig``.
"""
...
### Task 7: 网络注册 — 在各 model 文件内定义网络配置

**Files:**
- Modify: `src/qmp/models/hubbard.py`, `src/qmp/models/fcidump.py`, `src/qmp/models/openfermion.py`
- Modify: `tests/unit/models/test_hubbard.py`, `test_fcidump.py`, `test_openfermion.py`

沿用旧 worktree 组织方式：**网络构造参数写在具体 model 文件里**。每个 model 文件底部定义四个网络配置 dataclass 并注册进 `Model.network_dict`（键 `mlp/u1u1`, `mlp/u1`, `transformers/u1u1`, `transformers/u1`）。

- [ ] Step 1: 在每个 model 文件顶部 import 四个 WaveFunction 变体（MLP/Transformer × Electron/UpDown），`TYPE_CHECKING` 下 import `nnx` 与 `NetworkProto`。
- [ ] Step 2: 定义配置 dataclass：MLP 版字段 `hidden_size`, `ordering`；Transformer 版字段 `embedding_dim`, `heads_num`, `feed_forward_dim`, `depth`, `tail_hidden_dim`, `ordering`（带默认值）。每个含 `create(self, model, *, rngs: nnx.Rngs) -> NetworkProto`。
- [ ] Step 3: 自旋拆分——hubbard 用 `electron_number // 2` / `electron_number - electron_number // 2`；fcidump/openfermion 用 `(n_electrons ± n_spins) // 2`。
- [ ] Step 4: 底部 `Model.network_dict[name] = ConfigClass` 注册。
- [ ] Step 5: 测试：注册表映射、逐配置构造（全空间归一化 Σ|ψ|²=1）、粒子数守恒、generate/generate_unique 自洽、PRNG 确定性、ordering、配置字段透传、奇数电子数自旋拆分。删除过时的 `test_*_network_dict_empty`。
- [ ] Step 6: SOP + Commit（接口与文档/测试可分两次提交，接口先行供其他分支使用）。

**与旧代码差异**（架构迁移）：`create` 显式收 `rngs`（nnx 需 PRNG）；Transformer 去 MoE 参数、改用 dense `tail_hidden_dim`；去掉 `physical_dim`/`is_complex`；未迁移 MPS。网络配置类刻意在各 model 文件内重复（同「转发样板刻意不抽基类」原则）。
...
各 Task 的内联测试块是最小 TDD 版本 (Task 1/2/4/5 分别 2/7/3/5 个用例)。代码稳定后测试被系统性扩充，覆盖边界、错误路径、不变量、端到端数值与网络注册 (Task 7)。以实际提交的测试文件为准，本计划的内联块不再逐字同步。当前覆盖 (models 部分 100 个用例)：

- **test_model.py**：`model_dict` / `model_config_dict` 注册/检索；`importlib.import_module` 触发注册；三个 model 均已注册；参数化验证每个注册类实现完整 `ModelProto` 算符接口（含 `create_network`）。
- **test_build.py**：`SubConfigRef` 默认/带参；`build_model` None-ref、有效构造、默认参数、未知 name `KeyError`、dacite 类型错误、未知参数忽略、config 有 impl 无、可导入但未注册 `KeyError`。
- **test_create_network.py**：`create_network` 按 name 分发四个注册配置（构造归一化）、未知 name `KeyError`、dacite 反序列化（空参用默认 / 覆盖 / 错类型 DaciteError / 未知键忽略）、model 元数据注入（`electron_number` 驱动守恒）、PRNG 确定性、构造出的网络 NetworkProto 一致（generate / generate_unique 自洽）。
- **test_hubbard.py**：注册、`n_qubits`、半填充默认、显式 `electron_number`、`devices` 默认、配置校验错误 (维度非正 / 电子数越界高/负)、2×1 与 2×2 项计数 (横向+纵向 hopping、on-site)、化学势系数、`ref_energy` 透传、`show_config` 全占据渲染；端到端数值：on-site 对角 (U)、化学势对角 (−mu)、hopping 振幅 (−t)、`find_all`/`find_topk` 带 `exclude=None` 转发。网络 (Task 7)：`network_dict` 键映射、四配置构造归一化、u1/u1u1 守恒、默认超参、配置字段透传、PRNG 确定性、奇数电子自旋拆分、ordering、generate/generate_unique 自洽。
- **test_openfermion.py**：注册、元数据、`n_spins = multiplicity − 1`、缺 `fci_energy` 的 `ValueError`、算符转发。网络 (Task 7)：`network_dict` 键映射、四配置构造归一化、u1/u1u1 守恒、generate_unique 自洽、PRNG 确定性。
- **test_fcidump.py**：注册、元数据、`ref_energy` 默认 0 与显式、缓存命中、缓存文件写入、缓存内容 == 新鲜解析、不同内容不同缓存 key、`read_fcidump` headonly/完整/`.gz`/Psi4 单电子行忽略/非法格式 `ValueError`；端到端数值：闭壳层对角对拍手算值、真空对角为 0。网络 (Task 7)：`network_dict` 键映射、四配置构造归一化、u1/u1u1 守恒、generate_unique 自洽、PRNG 确定性、配置字段透传。

CLI 层（`tests/unit/test_cli.py`、`tests/unit/algorithms/test_demo.py`）覆盖 `create_network` 的端到端调用：Demo 经 `model.create_network` 构造网络并跑 `<H>`；`main()` 全链路无 mock 派发（YAML → build_model → create_network → generate → apply_within_subspace → run）；`main()` 对未注册 action 的 `KeyError`。`test_fcidump.py` / `test_openfermion.py` 另各含 `create_network` 分发 + 未知 name smoke 测试（因该方法在各 model 文件内逐字重复，需防单文件 typo）。

网络注册测试遵循「最小而有效」：构造正确性用小系统全空间枚举验证（归一化+守恒），避免昂贵的统计性采样测试（采样分布正确性已在 networks 子系统层验证）。

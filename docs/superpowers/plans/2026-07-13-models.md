...
**Architecture:** 每个 model 是薄封装：`ModelConfig` dataclass → `__init__` 构造哈密顿量字典 → `FermiHamiltonian(dict, n_qubits=..., devices=...)` → 转发四个算符方法 + `show_config` + 元数据。全局 `model_dict` + `model_config_dict` 注册表实现插件式发现。networks 尚未迁移，本轮 `network_dict` 留空。

...
各 Task 的内联测试块是最小 TDD 版本 (Task 1/2/4/5 分别 2/7/3/5 个用例)。代码稳定后测试被系统性扩充，覆盖边界、错误路径、不变量、端到端数值与网络注册 (Task 7)。以实际提交的测试文件为准，本计划的内联块不再逐字同步。当前覆盖 (models 部分 95 个用例)：
...
CLI 层（`tests/unit/test_cli.py`、`tests/unit/algorithms/test_demo.py`）覆盖 `create_network` 的端到端调用：Demo 经 `model.create_network` 构造网络并跑 `<H>`；`main()` 对未注册 action 的 `KeyError`。

...
├── _model.py            # ModelProto[ConfigT] 协议 + model_dict 注册表
├── hubbard.py           # 格点参数 → FermiHamiltonian (无外部依赖)
├── fcidump.py           # FCIDUMP 文件 → FermiHamiltonian (numpy 缓存)
...
def test_model_dict_registration() -> None:
    ...
    sentinel = object()
    model_dict["_test_entry"] = sentinel  # type: ignore[assignment]
    assert model_dict["_test_entry"] is sentinel
...
class ModelConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.electron_number < 0 or self.electron_number > 2 * self.m * self.n:
            raise ValueError(
                f"The electron number {self.electron_number} is out of bounds for a "
                f"{self.m}x{self.n} lattice."
            )
    ...
    def _prepare_hamiltonian(cls, config: ModelConfig) -> dict[tuple[tuple[int, int], ...], complex]:
        ...
        for i in range(config.m):
            for j in range(config.n):
                ...
                hamiltonian_dict[
                    (index(i, j, 0), 1), (index(i, j, 0), 0), (index(i, j, 1), 1), (index(i, j, 1), 0)
                ] = config.u
...
def test_openfermion_spin_from_multiplicity(tmp_path) -> None:
    ...
    geometry = [("H", (0.0, 0.0, 0.0)), ("H", (0.0, 0.0, 0.7414))]
    molecule = openfermion.MolecularData(
        geometry=geometry, basis="sto-3g", multiplicity=3, charge=0,
...
class Model(ModelProto[ModelConfig]):
    ...
    def __init__(self, config: ModelConfig) -> None:
        ...
        fermion_operator = openfermion.transforms.get_fermion_operator(
...
### Task 5: `fcidump.py` — FCIDUMP 文件模型 (带 numpy 缓存)
...
git commit -m "feat(models): add FCIDUMP model with numpy/pickle cache"
```
...
Expected: no errors. If `ty` complains that `ModelProto` lacks `config_t` for a construction site or that the Protocol generic is unused, resolve by adjusting annotations. Only if strictly required, add back a `config_t: ClassVar[type]` class attribute to each model and document why in a comment.

- [ ] **Step 4: Full test suite**

Run: `uv run pytest -v`
Expected: all tests pass (previous 66 + new models tests), 0 failures

...
CLI 层（`tests/unit/test_cli.py`）覆盖 `create_network` 的端到端调用：`main()` 全链路无 mock 派发（YAML → build_model → create_network → generate → apply_within_subspace → run）；`main()` 对未注册 action 的 `KeyError`。`test_fcidump.py` / `test_openfermion.py` 另各含 `create_network` 分发 + 未知 name smoke 测试（因该方法在各 model 文件内逐字重复，需防单文件 typo）。

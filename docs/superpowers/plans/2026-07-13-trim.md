...
**Architecture:** 新建 `src/qmp/algorithms/trim.py`，从 `haar.py` import 复用 Lanczos/采样/优化/checkpoint helper。新增三个核心函数 `_expand_pool`（多跳全局 top-K 扩展）、`_local_trim`（随机分块 Lanczos）、`_global_trim`（全局 Lanczos），以及 `TrimConfig`/`Trim`/主循环。

**Tech Stack:** jax, jax.numpy, flax.nnx, optax（间接），pytest, ruff, ty。

**参考 spec:** `docs/superpowers/specs/2026-07-13-trim-design.md`（**以 spec 为准**——下方任务内的代码块是初版草稿，实现阶段有如下修订）

**实现修订（spec 已更新，此处备忘）：**
- `_expand_pool`：每跳用 `_unique_configs` 去重（top-K 在小系统返回零填充行）。
- `_local_trim`：每块 `max_steps` 上界取 `block_dim - 1`，防止小 block 上零范数除法 NaN。
- 主循环终止/checkpoint 逻辑与 haar 对齐（固定 `start` 上界、`checkpoint_path is not None`、`self._state` 暴露）；期间修复了 haar 的死循环 + cwd 污染 bug，故 haar.py 也有改动。
...
两者主循环终止/checkpoint 逻辑一致（见 AGENTS「主循环约定」）。
